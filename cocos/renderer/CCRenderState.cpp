...
void RenderState::StateBlock::bindNoRestore()
{
    ...
    if ((_bits & RS_POLYGON_OFFSET) && (m_polygonOffsetEnabled != _defaultState->m_polygonOffsetEnabled || (m_polygonOffsetEnabled && m_offset != _defaultState->m_offset)))
    {
        if (m_polygonOffsetEnabled)
            glEnable(GL_POLYGON_OFFSET_FILL);
        else
            glDisable(GL_POLYGON_OFFSET_FILL);
        glPolygonOffset(m_offset.m_factor, m_offset.m_units);
        
        _defaultState->m_polygonOffsetEnabled = m_polygonOffsetEnabled;
        _defaultState->m_offset = m_offset;
    }
    if ((_bits & RS_CLIP_BOUNDS) && (m_clipEnabled != _defaultState->m_clipEnabled || (m_clipEnabled && !m_glBounds.equals(_defaultState->m_glBounds))))
    {
        if (m_clipEnabled)
        ...
        _defaultState->m_clipEnabled = m_clipEnabled;
        _defaultState->m_glBounds = m_glBounds;
    }
...
//    if (_defaultState->_bits == 0)
    ...
    /*BPC PATCH*/
    if (!(stateOverrideBits & RS_POLYGON_OFFSET) && (_defaultState->_bits & RS_POLYGON_OFFSET))
    {
        glDisable(GL_POLYGON_OFFSET_FILL);
        _defaultState->_bits &= ~RS_POLYGON_OFFSET;
        _defaultState->m_polygonOffsetEnabled = false;
    }
    if (!(stateOverrideBits & RS_CLIP_BOUNDS) && (_defaultState->_bits & RS_CLIP_BOUNDS))
    {
        glDisable(GL_SCISSOR_TEST);
        _defaultState->_bits &= ~RS_CLIP_BOUNDS;
        _defaultState->m_clipEnabled = false;
    }
    /*END BPC PATCH*/
}
...
void RenderState::StateBlock::cloneInto(StateBlock* state) const
{
    ...
    /*BPC PATCH*/
    state->m_clipEnabled = m_clipEnabled;
    state->m_glBounds = m_glBounds;
    state->m_polygonOffsetEnabled = m_polygonOffsetEnabled;
